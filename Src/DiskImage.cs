...
namespace CPM
{
    public class DiskImage
    {
        ...
        public DiskImage(string file_name, string volume, byte[] bytes, int max_dir)
        {
            this.file_name = file_name;
            this.volume = volume;
            this.bytes = bytes;
            ...
            if (volume.ToUpper() == "A")
            {
                DialogResult dialogResult = MessageBox.Show("Is this (or contains this container) a boot drive ?", "BOOT ?", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
